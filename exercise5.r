...
# http://stackoverflow.com/questions/9818125/identifying-duplicate-columns-in-anr-data - frame
...
set.seed(1234)
split <-
  sample(nrow(gisette_df_train_test), floor(0.5 * nrow(gisette_df_train_test)))
...
bestVars <-
  data.frame('features' = names(traindf)[solutions(feats)[[1]]],
             'scores' =
               scores(feats)[[1]])
 
library(caret)
traindf_temp <-
  traindf[c(as.character(bestVars$features), 'cluster')]
 
# caret requires a factor of non-numeric value
traindf_temp$cluster <-
  ifelse(traindf_temp$cluster == 1, "yes", "no")
...
glmnet_predictions <-
  predict(object = glmnet_model,
          newdata = gisette_df_validate[, as.character(bestVars$features)],
          type = 'raw')
# caret requires a factor of non-numeric value
gisette_df_validate$cluster <-
  ifelse(gisette_df_validate$cluster == 1, "yes", "n
         o")
gisette_df_validate$cluster <-
  as.factor(gisette_df_validate$cluster)
...
knn_predictions <-
  predict(object = knn_model,
          newdata = gisette_df_validate[, as.character(bestVars$features)],
          type = 'raw')
print(postResample(
  pred = knn_predictions,
  obs = as.factor(gisette_df_validate$cluster)
))
# ensemble example
feats <-
  mRMR.ensemble(
